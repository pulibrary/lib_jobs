...
RSpec.describe Oclc::NewlyCatalogedJob, type: :model do
  ...
  before do
    ...
    allow(sftp_session).to receive(:download!).with(file_full_path).and_return(Rails.root.join('spec', 'fixtures', 'oclc', file_name_to_download).read)
    allow(Net::SFTP).to receive(:start).and_yield(sftp_session)
  end
  it 'downloads only the relevant files' do
    expect(newly_cataloged_job.run).to be_truthy
    expect(sftp_session).to have_received(:download!).with(file_full_path)
  end
end

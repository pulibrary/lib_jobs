...
RSpec.describe AlmaFundAdjustment::FundAdjustment do
  subject(:result) do
    described_class.new.call({ "FUND_EXTERNAL_ID" => "12345|Z9999",
                               "FISCAL_PERIOD_ID" => "22",
                               "AMOUNT" => "13.36",
  ...
  describe "#unique_id" do
    it "cobines the external id and the note" do
      expect(result.unique_id).to eq("123456-Online Entry | A Fund We have | 07/14/2021")
    end
  end

  describe "#original_amount" do
    it "gives the original amount" do
      expect(result.original_amount).to eq("13.36")
    end
  end

  describe "#transposed_amount" do
    it "returns the negative of the original" do
      expect(result.transposed_amount).to eq("-13.36")
    end
    ...
    context "A zero amount" do
      ...
      it "returns the original" do
        expect(result.transposed_amount).to eq("0.00")
      end
    end
  end
  ...
  describe "#adjusted_row" do
    it "contains the transpoxed amount" do
      expect(result.adjusted_row["AMOUNT"]).to eq("-13.36")
    end
  end
end
